...
void fft_init(struct_FFT *fft, int n){
...
void fft_free(struct_FFT *fft){
    free(fft->reversed);
    free(fft->c);
    free(fft->s);
}

static void fft_swap_and_first_pass(struct_FFT *fft, float *re, float *im){
    int i, n = fft->n;
    for (i = 0; i < n; i++){
...
void fft_fft(struct_FFT *fft, float *re, float *im){
